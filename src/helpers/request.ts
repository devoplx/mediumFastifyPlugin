...
const request = <T>(path: string, method: string = "GET", headers: { [key: string]: string } = {}): Promise<MyResponse<T> | FastifyError> => {
  return new Promise((resolve, reject) => {
    ...
    const req = https.request(options, async (res: any) => {
      if (res.statusCode >= 300 && res.statusCode < 400 && res.headers.location) {
        try {
			const data = await request<T>(res.headers.location, method, headers)
        ...
        let data = "";
		let response: MyResponse<T> = { statusCode: res.statusCode, data: {} as T };
